...
def get_correspondences(A1_ds,A2_ds,A1_pts,A2_pts,w,pad=False):
    ...
    for p,q in zip(A1_pts.astype(int),A2_pts.astype(int)):
        ...
        p1,p2 = get_point_correspondence(p,q,A1sub,A2sub,autocorrelation_threshold=0.8,padarray=pad,value_threshold=1000)
        if not p1 is None:
            pm1.append(p1)
            pm2.append(p2)
    if pm1:
