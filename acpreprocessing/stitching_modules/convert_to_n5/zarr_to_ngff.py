...
from acpreprocessing.stitching_modules.acstitch.zarrutils import get_zarr_group
...
def write_zarr_to_zarr(
        zarr_fn, zarr_grp, output_n5, group_names, group_attributes=None, max_mip=0,
    ...
    zgrp = get_zarr_group(zpath=zarr_fn,grpname=zarr_grp)
    dataset = zgrp[0]
    ...
    if numchunks < 1:
        ...
        if deskew_options and deskew_options["deskew_transpose"]:
            # input dataset must be transposed
            joined_shapes = (dshape[0],numchunks*block_size[1],dshape[1])
        else:
            joined_shapes = (dshape[0],numchunks*block_size[1],dshape[2])
...
def zarr_to_ngff_group(zarr_fn, zarr_grp, output, *args, **kwargs):
    ...
    if output == 'zarr':
        ...
        return write_zarr_to_zarr(zarr_fn, zarr_grp, *args, **kwargs)
    else:
        print('unknown output format: ' + output)



class ZarrToNGFFParameters(NGFFGroupGenerationParameters):
    input_file = argschema.fields.Str(required=True)
    input_group = argschema.fields.Str(required=True)
...
class ZarrToZarrInputParameters(argschema.ArgSchema,
                                    ZarrToNGFFParameters):
...
class ZarrToZarr(argschema.ArgSchemaParser):
    default_schema = ZarrToZarrInputParameters

    def run(self):
        deskew_options = (self.args["deskew_options"]
                          if "deskew_options" in self.args else {})
        zarr_to_ngff_group(
            self.args["input_file"], self.args["input_group"], self.args["output_format"],
